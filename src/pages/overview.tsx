        ...
        <>
            <div className="overview-wrapper">
                <div className="balance">
                    <div>
                        <span>Balance</span>
                        <span data-testid="balance">${income && expenses ? ((income + expenses) / 100).toFixed(2) : 0}</span>
                    </div>
                    <div>
                        <span>Income</span>
                        <span data-testid="income">${income ? (income / 100).toFixed(2) : ''}</span>
                    </div>
                    <div>
                        <span>Expenses</span>
                        <span data-testid="expenses">${expenses ? Math.abs(expenses / 100).toFixed(2) : ''}</span>
                    </div>
                </div>
                {/* pots */}
                <div className="pots">
                    ...
                    {Object.keys(overviewData).includes('pots') &&
                        <div>
                            <div className="total">
                                ...
                                <span data-testid="pots-total">${Math.floor(overviewData.pots.reduce((a, c) => a + c.total, 0)) / 100}</span>
                            </div>
                            ...
                            <ul>
                                {overviewData.pots.slice(0, 4).map(pot =>
                                    <li data-testid='pot-item' key={pot.name}>
                                        ...
                                        <span>{pot.name}</span>
                                        <span data-testid="pot-total">${Math.floor(pot.total) / 100}</span>
                                    </li>
                                )}
                            </ul>
                        </div>
                    ...
                    <ul>
                        {Object.keys(overviewData).includes('recent_transactions') && overviewData.recent_transactions.map(item =>
                            <li data-testid="transaction" key={item.date}>
                                <div>
                                    {/* img* */}
                                    {/* <img src="./src/assets/images/avatars/bytewise.jpg" alt="" /> */}
                                    <span>{item.name}</span>
                                </div>
                                <div>
                                    {/* format amount +/-* */}
                            ...
                            <ul>
                                {overviewData.budgets.slice(0, 4).map(budget =>
                                    <li key={budget.category} >
                                        ...
                                        <span data-testid="budget-maximum">${(budget.maximum / 100).toFixed(2)}</span>
                                    </li>
                                )}
                            </ul>
                        </div>
