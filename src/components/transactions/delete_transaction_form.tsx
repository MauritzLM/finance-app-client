...
export default function DeleteTransactionForm({ user, selected_transaction, hideDeleteForm }: deleteFormProps) {
    ...
    return (
        <form data-testid="delete-form" className="cs-form delete-form" action="" onSubmit={(e) => handleSubmit(e)}>
            <h2>Delete Transaction</h2>
            <button data-testid="close-btn" className="close-btn" type="button" onClick={hideDeleteForm}>
            ...
            <p data-testid="message">Are you sure you want to delete <span>{`${selected_transaction.name} - ${new Date(selected_transaction.date).toLocaleString('en-GB', { 'day': 'numeric', 'month': 'short', 'year': 'numeric' })}`}</span>? This cannot be reversed!</p>
