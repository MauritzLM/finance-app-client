...
import { categories, avatar_images } from "../../data/data"
...
export default function NewTransactionForm({ user, recurringBills, updateRecurringBills, hideNewForm }: newFormProps) {
    // controlled inputs*
    const [formData, setFormData] = useState({ 'avatar': '', 'name': '', 'category': '', 'date': '', 'amount': 0, 'recurring': false })
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL}/finance-api/transactions/create`, {
                method: 'POST',
                headers: {
    ...
    return (
        <form className="cs-form" action="" onSubmit={(e) => handleSubmit(e)}>
            ...
            <div className="form-group">
                ...
                <div>
                    ...
                    <select name="avatar" id="avatar" value={formData.avatar} onChange={(e) => setFormData({ ...formData, 'avatar': e.currentTarget.value })}>
                        <option value="">Select an avatar</option>
                        {avatar_images.map((a, i) =>
                            <option key={`${a}-${i}`} value={a}>{a.split('/')[3]}</option>
                        )}
                    </select>
                </div>
            </div>
            ...
            <div className="form-group">
                ...
                <input type="date" name="date" id="date" value={formData.date} onInput={(e) => setFormData({ ...formData, 'date': e.currentTarget.value })} />
            </div>
            ...
            <div className="form-group">
                ...
                <input type="number" name="amount" id="amount" value={formData.amount} onInput={(e) => setFormData({ ...formData, 'amount': Number(e.currentTarget.value) })} />
            ...
            <div className="form-group checkbox-group">
                ...
                <input type="checkbox" name="recurring" id="recurring" checked={formData.recurring} onChange={(e) => setFormData({ ...formData, 'recurring': e.currentTarget.checked })} />
            </div>
