...
function WithdrawForm({ user, pot, pots, updatePots, hideWithdrawForm }: withdrawFormProps) {
    ...
    return (
        <>
            <form className="cs-form withdraw-form" data-testid="withdraw-form" onSubmit={(e) => handleSubmit(e)}>
                <h2>Withdraw from '{pot.name}'</h2>
                <button className="close-btn" type="button" onClick={hideWithdrawForm}>
                ...
                <div className="total"><span>New Amount</span> <span data-testid="new-amount">${((pot.total / 100) - formData.amount).toFixed(2)}</span></div>
                {/* bar* */}
