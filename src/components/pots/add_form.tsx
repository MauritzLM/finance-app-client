    ...
    return (
        <>
            <form data-testid="add-form" onSubmit={(e) => handleSubmit(e)}>
                ...
                <div><span>New Amount</span> <span data-testid="new-amount">${(formData.amount + (pot.total / 100)).toFixed(2)}</span></div>
                {/* bar */}
                <div className="progress-bar">
                    <div className="initial-value" style={{ width: `${roundPercentage((pot.total / pot.target) * 100)}%` }}></div>
                    <div className="new-value" style={{ width: `${roundPercentage(((formData.amount + (pot.total / 100)) / (pot.target / 100)) * 100)}%` }}></div>
                </div>
                <div><span data-testid="percentage">{roundPercentage(((formData.amount + (pot.total / 100)) / (pot.target / 100)) * 100)}%</span> <span data-testid="target">Target of ${pot.target / 100}</span></div>
