...
function AddForm({ user, pot, pots, updatePots, hideAddForm }: addFormProps) {
    ...
    return (
        <>
            <form className="cs-form add-form" data-testid="add-form" onSubmit={(e) => handleSubmit(e)}>
                <h2>Add to '{pot.name}'</h2>
                <button className="close-btn" type="button" onClick={hideAddForm}>
                ...
                <div className="total"><span>New Amount</span> <span data-testid="new-amount">${(formData.amount + (pot.total / 100)).toFixed(2)}</span></div>
                {/* bar */}
